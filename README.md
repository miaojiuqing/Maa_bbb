...
## PC端使用须知
  ...
  PC端用户请注意：自动战斗功能运行战斗时，请一定不要点击键盘上的特殊键(如win、ctrl、alt)等，**⚠️否则可能会导致整个电脑界面被各种各样快捷键唤出的窗口占满导致无法操作！！！**
...
- ✅ 克莱茵打灰（刷装甲等级/乐土币/萌新刷命定歧路）| 此任务自己会找路，没必要和自动乐土放一起
  - ✅ 减负模式-无限刷
  - ✅ 浅层序列刷
  - ✅ 首层复刷
  - ✅ 无限刷取模式
...
- ✅ 自动战斗
  - ✅ 乐土版
    ...
    - ✅ 薇塔(开环流)
    - ❌ 新春虫(开环流)
    - ✅ 崩落符(普攻流)-PC用，模拟器不建议用，A太慢了，严格来说自动战斗这个功能目前是不推荐模拟器用户去使用的
    - ✅ 嗨♪爱愿妖精(开环流)
  - ❌ 超弦空间/量子流行版
    - ❌ 虚三家
- ✅ 自动过剧情
  - ✅ 纯点击跳过（就是看到跳过就会点击，会自己点掉今日不再显示的提示可能会错过CG收集）
  - ✅ 无视跳过正常过剧情-手动选择选项（正常过剧情，遇到对话选项会停）
  - ✅ 无视跳过正常过剧情-自动选择选项（正常过剧情，遇到对话选项会自己选择最上面的）
  - ✅ 古早剧情的点击跳过（很老的那种，两个悬浮窗在那）
...
Q:我运行了安装目录中的依赖库安装后，软件与cil均双击无法启动，但shift+右键命令行输入./MFAAvalonia.exe可以启动是为什么

A:邪恶的微软犯病了，请右键MFAAvalonia.exe->属性->最下面的安全->勾选 解除锁定 后再次尝试
...
0. [MaaFramework](https://github.com/MaaXYZ/MaaFramework)伟大无需多言，没有MaaFramework就没有MAA_bbb
1. [M9A](https://github.com/MAA1999/M9A)借鉴学习了不少思路，不愧是最佳实践.jpg
2. [HonkaiHelper](https://github.com/Aues6uen11Z/HonkaiHelper)我觉得这个项目很棒，学习了前置准备和一些日常任务完成方式
3. [MAA_Punish](https://github.com/overflow65537/MAA_Punish)同款自动战斗，原汁原味（因为是E佬写的战斗框架所以包原汁原味的）
4. [Maaend](https://github.com/MaaEnd/MaaEnd)乐土的自动寻门参考
...
Windows **x64 自解压安装程序**在构建时使用了 **7-Zip** 的 **`7zSD.sfx`** 模块（版权 **Igor Pavlov**，以 **GNU LGPL** 等为准）。仓库内已放置第三方说明与 7-Zip 官方许可副本：

- [`LICENSE_thirdparty.md`](LICENSE_thirdparty.md)（入口说明）
- [`third_party/7-zip/7zSD.sfx`](third_party/7-zip/7zSD.sfx)（构建用自解压模块，与声明同目录）
- [`third_party/7-zip/LICENSE_thirdparty.md`](third_party/7-zip/LICENSE_thirdparty.md)
- [`third_party/7-zip/7-Zip_License.txt`](third_party/7-zip/7-Zip_License.txt)（来源：<https://www.7-zip.org/license.txt>）

**最终用户安装/解压目录**中会包含 `third_party/7-zip/` 下的 **`LICENSE_thirdparty.md`** 与 **`7-Zip_License.txt`**（不含单独的 `7zSD.sfx`，该模块已并入自解压 `.exe`）。GitHub Releases 的正文亦附带简要说明。本仓库主体代码仍以根目录 **`LICENSE`（MIT）** 为准；MIT 不适用于 7-Zip 二进制组件。
