...
- `assets/resource/pipeline/战斗逻辑/自动战斗按键.json`
  - 定义了：`攻击_action`、`长按攻击_action`、`闪避_action`、`武器技_action`、`必杀_action`、`qte1`/`qte2` 等
- 电脑端会用按键版覆盖：
  - `assets/resource_win32/pipeline/战斗逻辑/自动战斗按键.json`
  - 节点名不变，只是底层从"点击屏幕"变成"按键盘"

通用识别节点（判断"是否在战斗中"、读取血量/能量等）：

- `assets/resource/pipeline/战斗逻辑/自动战斗工具方法.json`
  - 包含：`战斗中`、`技能_能量条`、`技能_必杀能量`、`检查血量百分比`、`在战斗中检查角色`、`本关卡无法复活` 等

角色专属识别（可选，按需新增）：

- 建议放在 `assets/resource/pipeline/战斗逻辑/战斗逻辑/<角色名>.json`
  - 例如：`真理之律者.json`
...
| `long_ultimate_skill(等待毫秒数)` | 长按释放必杀技/大招 | 同上 |
...
这个方法的作用是"拿当前画面去匹配一个你事先定义好的识别规则"。识别规则写在资源文件（`.json`）里，可以是图片比对、颜色检测等方式。
...
推荐在 `assets/resource/pipeline/战斗逻辑/战斗逻辑/` 下新建一个 `<角色名>.json` 文件。
...
3. **识别区域是否正确**：`自动战斗工具方法.json` 里 `在战斗中检查角色` 的识别区域是否覆盖到了攻击键位置
